  ...
  (() => {
    try {
      ...
      const TIMEOUT_TRACKING_CONSTANTS = {
        ...
        // Timers between 500 ms and 6 seconds will be ignored if their stack
        // trace contains anything in this blacklist
        BLACKLIST: ["node_modules/firebase", "PageActivityTracker"],
      };
      ...
      window.setTimeout = (fnOrCode, delay = 0, ...params) => {
        ...
        const shouldTrack = (() => {
          if (delay > TIMEOUT_TRACKING_CONSTANTS.MAX_DELAY_MS) {
            ...
            // If the timer is between 500 ms and 6 seconds, check its stacktrace
            // against our blacklist
            ({stack} = new Error());
            if (
              TIMEOUT_TRACKING_CONSTANTS.BLACKLIST.some(forbidden =>
